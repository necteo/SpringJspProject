		...
		<div class="row">
		  	...
		  	<tbody>
		  	  <tr>
		  	  	<td class="text-center" colspan="3">
		  	  		<img :src="store.detail.vo.poster" style="width: 800px; height: 250px">
		  	  	</td>
		  	  </tr>
		  	  <tr>
		  	   	<td class="text-center" colspan="3">
		  ...
		  <table class="table">
		  	<tbody>
		  		...
		  		<tr v-for="(data, idx) in store.detail.tList" :key="idx">
		  			<td class="text-left" width="85%">{{data}}</td>
		  			<td class="text-right" width="15%"><img :src="store.detail.iList[idx]" style="width: 180px; height: 100px"></td>
		  		</tr>
		  	</tbody>
		  </table>
		  <table class="table">
		...
		<div class="row" style="margin-top: 20px" id="recipe_reply">
			<table class="table">
				<tbody>
					<tr>
						<td>
							<table class="table" v-for="(rvo, idx) in rStore.reply_list" :key="idx">
								<tbody>
									<tr>
										...
										<td class="text-right">
											<button class="btn-xs btn-success" v-if="rStore.sessionId === rvo.id" @click="rStore.toggleUpdate(rvo.no, rvo.msg)">
												{{rStore.upReplyNo === rvo.no ? '취소' : '수정'}}</button>
